...
{"id": "<原样返回的 id>", "track": "<一级赛道>", "directions": ["<二级研究方向 id，0-2 个>"], "keywords": ["<技术关键词，最多 5 个>"], "relevance": <1-5 整数>, "title_zh": "<中文短标题，≤20字>", "improvement": "<一句话改进点提炼>"}
...
【title_zh】准确概括内容的中文短标题，≤20 字，不标题党，保留关键专有名词（如模型名）。
...
输出元素: {"id":"a1","track":"foundation-model","directions":["model-arch"],"keywords":["MoE","scaling"],"relevance":4,"title_zh":"FooLM：MoE 架构扩展至万亿参数","improvement":"以 MoE 架构在 MMLU 上超越稠密基线 12%，同时降低 40% 计算量"}
输入: {"id":"b2","source":"blog","title":"Acme Corp transforms customer service with GPT","content":"Case study about how Acme uses our API..."}
输出元素: {"id":"b2","track":"application","directions":[],"keywords":["case study"],"relevance":1,"title_zh":"Acme 客服场景客户案例","improvement":"发布 Acme 公司使用 API 改造客服的客户案例"}`;
...
async function classifyBatch(batch: InsightItem[]): Promise<void> {
  ...
  for (const item of batch) {
    ...
    item.ai_tags = {
      ...
      title_zh: r.title_zh ? String(r.title_zh).slice(0, 40) : undefined,
