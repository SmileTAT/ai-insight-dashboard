...
`WINDOW_DAYS`（采集窗口，默认 7）、`ARXIV_LLM_LIMIT`（每周进入 LLM 分析的论文上限，默认 50）、`ARXIV_CANDIDATE_LIMIT`（粗筛候选池上限，默认 200）、`ARXIV_MAX_PAGES`（API 分页上限，默认 10）、`FOCUS_DIRECTIONS`（关注的研究方向，默认聚焦 Agent 全部细分方向 + 记忆，见 `src/config.ts`，命中方向在周报置顶）、`MIN_REPORT_RELEVANCE`（进入报告的最低战略相关性 1-5，默认 3，低于阈值只归档）。
